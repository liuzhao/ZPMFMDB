...
/**
 单例方法创建数据库, 如果使用shareDatabase创建,则默认在NSDocumentDirectory下创建ZPMFMDB.sqlite, 但只要使用这三个方法任意一个创建成功, 之后即可使用三个中任意一个方法获得同一个实例,参数可随意或nil
 
 dbName 数据库的名称 如: @"Users.sqlite", 如果dbName = nil,则默认dbName=@"ZPMFMDB.sqlite"
...
/**
 ...
 p.name = @"小李";
 [jqdb zpm_inDatabase:^{
 [jqdb zpm_insertTable:@"users" dicOrModel:p];
 }];
 */
...
/**
 ...
 [jq zpm_inTransaction:^(BOOL *rollback) {
 BOOL flag = [jq zpm_insertTable:@"users" dicOrModel:p];
 if (!flag) {
